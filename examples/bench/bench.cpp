...
PrefData RecordPrefData(struct llama_context * ctx){
    PrefData data;
    llama_timings perf_data = llama_get_timings(ctx);
...
void printPrefData(std::vector<PrefData> pref_data){
    ...
    for (size_t i = 0; i < pref_data.size(); ++i) {
        ...
        printf("prompt %zu: prefill throughput = %.2f t/s, decode throughput = %.2f t/s %s\n",
    ...
    printf("\nTotal:\n");
    printf("prefill throughput = %.2f t/s, decode throughput = %.2f t/s\n",
...
int main(int argc, char ** argv) {
    ...
    for(const std::string p : prompts) {
        prompt_tokens.push_back(llama_tokenize(ctx, p, add_bos, true));
    }
